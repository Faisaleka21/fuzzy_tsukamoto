  ...
  const renderTopbar = (session) => {
    ...
    topbar.innerHTML = `
      <div style="display: flex; align-items: center;">
        <button class="mobile-menu-toggle" data-action="toggle-menu" aria-label="Toggle menu">☰</button>
    ...
    const buildOptions = (variabel, selectedValue) =>
      (state.bobotOptions[variabel] || [])
        .map((option) => {
          ...
          return `<option value="${option.value}" ${selected}>${option.label} - Nilai: ${option.value}</option>`;
        })
        .join("");
